...
void gpssensor(){
  ...
  // display data to the Wemos D1 Mini serial monitor
  Serial.println("Retrieve serial data from Arduino Pro Micro board..."); 
...
void telegrambot(){
  ...
  if (currentMillis - previousMillis >= interval) { // if the current time minus the previous time is greater than equal to the interval then
    ...
    if(myBot.getNewMessage(msg)){ // if there is an incoming message...    
      if(msg.messageType == CTBotMessageText){ // checking the type of message received
